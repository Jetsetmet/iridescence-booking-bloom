...
BEGIN
  ...
  IF EXISTS (
    ...
    VALUES (message_id, 'lead-magnet', normalized_email, 'suppressed', 'Recipient is suppressed');
    RETURN NEW;
  END IF;

  SELECT token INTO unsubscribe_token
  FROM public.email_unsubscribe_tokens
  WHERE email = normalized_email AND used_at IS NULL
  LIMIT 1;
  ...
  IF unsubscribe_token IS NULL THEN
    INSERT INTO public.email_send_log (message_id, template_name, recipient_email, status, error_message)
    VALUES (message_id, 'lead-magnet', normalized_email, 'suppressed', 'Recipient has unsubscribed');
    RETURN NEW;
  END IF;
  ...
  email_html := concat(
    ...
    '<div style="text-align:center;margin:28px 0;"><a href="https://www.youtube.com/watch?v=FmXDAoPIr_0" style="background:#a47148;color:#ffffff;padding:14px 28px;border-radius:999px;font-size:15px;text-decoration:none;display:inline-block;">Begin the practice</a></div>',
  ...
  VALUES (message_id, 'lead-magnet', normalized_email, 'pending');
